  ...
  render() {
    ...
    this.container.innerHTML = `
      <div class="trace-terminal">
        ...
        <p class="trace-evidence-note">Authored teaching example. The states below are not captured test results or live protocol data.</p>
        ...
        <div class="trace-body-grid">
          ...
          <div class="trace-panel action-panel">
            ...
            <div class="invariants-evaluated-list">
              <div class="panel-tag" style="margin-top: 16px;">REFERENCED MODEL PROPERTIES</div>
              ${(step.invariantsChecked || []).map(inv => `
                <div class="inv-eval-badge">
                  <span>·</span>
                  <code>${inv}</code>
                  <span class="eval-status">NOT EVALUATED</span>
                </div>
              `).join('')}
            </div>
          </div>
